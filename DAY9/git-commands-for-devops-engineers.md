...
How to create or initialize a git repo - 
git init, 
ls -a, 
ls -ltra, 
.git is a key component for the git, 
as it provides the tools to track the changes made and also keep the passwords etc protected and protects them to get committed to the github
...
git pull origin master --rebase && git push -u origin masterthese 2 commands which asked me to give my github username and the PAT which I had to generate from the classic token generation option 
...
git push -u origin masterWhat it did
...
ex - calculator application is delivered, now their is a new requirement which needs to be added the calculator application which is very hugefor a new feature, create a new branch, so that we don’t touch the main branch in order to keep the existing application stable and running without any issues.
